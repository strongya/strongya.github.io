...
import { a as getCleanSlug, b as getPostPath, g as getPostUrl } from '../../../chunks/urls_zT8Wsznn.mjs';
export { renderers } from '../../../renderers.mjs';

async function getStaticPaths() {
  if (process.env.BUILD_TARGET !== "github") return [];
  const posts = await getCollection("posts");
  return posts.filter((post) => post.data.language === "en").map((post) => ({
...
const GET = async ({ params, site }) => {
  ...
  const post = posts.find((p) => {
    if (p.data.language !== "en") return false;
    return getCleanSlug(p) === slug;
  });
  if (!post) {
    return new Response(JSON.stringify({ error: "English version not available" }), {
      status: 404,
      headers: { "Content-Type": "application/json" }
    });
  }
  const baseUrl = site?.toString() || "https://strongya.github.io";
  const canonicalUrl = new URL(getPostPath(post), baseUrl).toString();
